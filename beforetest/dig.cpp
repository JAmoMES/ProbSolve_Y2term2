...
int bfs(pair<int,int> start,pair<int,int> stop,int times){
    ...
    while(!q.empty()){
        ...
        q.pop();
        if(level[times][x.first-1][x.second]==0&& (a[x.first-1][x.second] == '.'||a[x.first-1][x.second] == '|') && a[x.first][x.second] != '-' ){
        ...
        if(level[times][x.first][x.second-1]==0 && (a[x.first][x.second-1] == '.'||a[x.first][x.second-1] == '-') && a[x.first][x.second] != '|' ){
